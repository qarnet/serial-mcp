...
impl SerialHandler {
    ...
    #[tool(
        description = "Subscribe to a connection: starts a background stream that forwards received bytes as MCP `notifications/message` events with logger=\"serial:<connection_id>\". When timeout_ms is set, the stream auto-stops after that duration. When omitted, the stream runs until unsubscribe, connection close, or error. Set no_new_rx_timeout_ms to stop after a period of silence. With match, detects the first byte pattern. With rx_framing, emits per-frame notifications with structured data (line, delimiter, length-prefixed, SLIP, start/end marker). With rx_parser, interprets frame content (AT commands, JSON lines, shell prompts) — rx_parser is a sibling to rx_framing. With validate: true, checksum-mismatched frames are dropped and counted in the final stop notification's frames_dropped field. A malformed SLIP escape sequence emits a final notification with stop_reason=framing_error and an error field, carrying partial results. Replaces any prior subscription on the same connection. A final notification with stop_reason, truncated, bytes_observed, bytes_returned, elapsed_ms, frames_dropped is emitted when the stream ends.",
