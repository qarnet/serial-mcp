...
- **Shared tool catalog**: `server::tool_catalog()` returns the exact 26 `rmcp::model::Tool` attrs served by MCP; schema tests and the xtask evaluator consume it (exact-count test `tool_catalog_has_exactly_twenty_six_tools` guards drift).
