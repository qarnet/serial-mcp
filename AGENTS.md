...
- The release workflow runs when CI completes successfully on `main` (and on manual `workflow_dispatch`, which is a build-only dry-run). It derives `v<version>` from `Cargo.toml`. If that version already has a **published** GitHub release, the run skips entirely (a leftover draft from a failed run is retryable, not skipped).
- Otherwise it creates a draft release, builds the four platform binaries, attaches them to the draft, then seals it (publishing is what creates the tag under immutable releases), and publishes the crate — an already-published crates.io version is a no-op skip.
...
- **Pinned official conformance gate.** CI `mcp-conformance` Ubuntu job
  (15-min bound, `contents: read`) owns only environment setup, the bound,
  and report upload; ALL compatibility execution delegates to the shared
  runner `scripts/test-mcp-compat.sh` (`set -euo pipefail`, GNU `timeout`
  per fixture/conformance invocation): Node 22.19.0 (exact), Rust 1.97.1 +
  ...
  `target/conformance-results/` (one `<scenario>-2025-11-25` /
