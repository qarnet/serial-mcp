...
- `read` and `subscribe` share stop-reason vocabulary via `RxStopController`. Both tools read from the ring via cursors: `read` advances a shared cursor; `subscribe` has per-call private cursors via the `from` parameter (`now`/`cursor`/`buffer_start`/`{"offset": N}`). Subscriptions do NOT move the shared read cursor.
...
- `from` parameter on `subscribe` replays buffered history: `"now"` (default, live edge), `"cursor"` (shared read cursor), `"buffer_start"` (oldest retained byte), or `{"offset": N}` (absolute). Replayed history flows through the same framing/match pipeline as live data.
