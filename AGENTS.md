...
- **Release guarantee:** `ResetReleaseGuard` (modeled on `BreakResetGuard`) is armed with the configured release state BEFORE assertion; every explicit path releases and disarms only on success; drop spawns a best-effort release. A closed/disconnected port counts as released. Assertion/release I/O failure = tool error with cleanup attempted.
