...
fn optional_usize_as_u64() -> impl Strategy<Value = Option<u64>> {
    prop::option::of(any::<usize>().prop_map(|v| v as u64))
}
...
proptest! {
    ...
    #[test]
    fn wait_for_args_roundtrip(
        id in opaque_id(),
        pattern in r"[A-Za-z0-9_\r\n\t ]{1,256}",
        pat_enc in valid_encoding(),
        timeout in any_u64(),
        max_bytes in any_usize(),
        resp_enc in valid_encoding(),
    ) {
...
proptest! {
    ...
    fn open_result_schema_valid(
        ...
        let r = OpenResult { connection_id: id, port, baud_rate: baud };
    ...
    fn close_result_schema_valid(id in opaque_id()) {
        let r = CloseResult { connection_id: id };
        let v = serde_json::to_value(&r).unwrap();
    ...
    fn write_result_schema_valid(id in opaque_id(), bw in any_usize(), enc in valid_encoding()) {
        let r = WriteResult { connection_id: id, bytes_written: bw, encoding: enc };
        let v = serde_json::to_value(&r).unwrap();
    ...
    fn read_result_schema_valid(
        ...
        let r = ReadResult { connection_id: id, bytes_read: br, encoding: enc, data, timeout_ms: timeout, elapsed_ms: elapsed };
        let v = serde_json::to_value(&r).unwrap();
    ...
    fn set_dtr_rts_result_schema_valid(id in opaque_id(), dtr: bool, rts: bool) {
        let r = SetDtrRtsResult { connection_id: id, dtr, rts };
        let v = serde_json::to_value(&r).unwrap();
    ...
    fn send_break_result_schema_valid(id in opaque_id(), dur in any_u64(), actual in any_u64()) {
        let r = SendBreakResult { connection_id: id, duration_ms: dur, actual_duration_ms: actual };
        let v = serde_json::to_value(&r).unwrap();
    ...
    fn subscribe_result_schema_valid(
        ...
        chunk in any_usize(), poll in any_u64(), replaced: bool,
    ) {
        // Fire-and-forget: all Optional fields are null
        let r = SubscribeResult {
            connection_id: id.clone(), encoding: enc.clone(),
            max_chunk_bytes: chunk, poll_interval_ms: poll,
    ...
    fn unsubscribe_result_schema_valid(id in opaque_id(), was_active: bool) {
        let r = UnsubscribeResult { connection_id: id, was_active };
        let v = serde_json::to_value(&r).unwrap();
        assert_schema_valid!(UnsubscribeResult, v);
    }

    #[test]
    fn wait_for_result_schema_valid(
        id in opaque_id(), matched: bool, data in non_empty_string(),
        br in any_usize(), match_idx in optional_usize_as_u64(),
        timeout in any_u64(), resp_enc in valid_encoding(),
    ) {
        let r = WaitForResult {
            connection_id: id, matched, data, bytes_read: br,
            match_index: match_idx.map(|v| v as usize),
            timeout_ms: timeout, response_encoding: resp_enc,
        };
        let v = serde_json::to_value(&r).unwrap();
        assert_schema_valid!(WaitForResult, v);
    }
}
...
proptest! {
    ...
    fn clamp_or_err_with_known_limits(value in any_usize()) {
        let _ = clamp_or_err("read.max_bytes", value, MAX_READ_BYTES);
        let _ = clamp_or_err("wait_for.max_bytes", value, MAX_WAIT_BYTES);
        let _ = clamp_or_err("write.data.len()", value, MAX_WRITE_BYTES);
        let _ = clamp_or_err("subscribe.max_chunk_bytes", value, MAX_STREAM_CHUNK_BYTES);
    }
...
// ── Phase A.5: WaitForArgs empty-pattern edge case ──────────────────────────

#[test]
fn wait_for_validates_empty_pattern() {
    let args = WaitForArgs {
    ...
    let rt: WaitForArgs = serde_json::from_str(&json).unwrap();
    assert!(rt.pattern.is_empty(), "empty pattern must roundtrip");
}
...
fn all_result_types_have_valid_schema() {
    let types: Vec<(&str, Value)> = vec![
        ...
        ("WaitForResult", schemars_to_jsonschema::<WaitForResult>()),
    ];
